...
contract Console {
  ...
  function log(uint[] memory xx) public {
      for (uint i; i < xx.length; i++) {
          emit _TruffleConsoleLog(xx[i]);
      }
  }

  function log(bytes32 x, uint[] memory yy) public {
      for (uint i; i < yy.length; i++) {
          emit _TruffleConsoleLogNamed(x, yy[i]);
      }
  }
